...
let types = Object.values(hunt_2016[46]).filter( (value,indexValue) => indexValue>2  && indexValue != 29 ).map(value => Number(value)),
      
/*
  END:Блок получения выборки данных за перод с 2016 по 2019 годы
*/

areasName = Object.keys(hunt_2019[45]).filter( (value,indexValue) => indexValue>2 && indexValue != 29);
